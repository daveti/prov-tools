...
int plog_process(FILE *log, plog_handler_t *fns[], int nfns)
{
	...
	/* this portion is for spade_handler. Please comment it if not using spade_handler*/
		...
		if (header.type < nfns && fns[header.type]){				 
		  if(header.type==0)
		 {
			...
			num_events++;
				...
				// filter the based on credid to prepare the graph
				unsigned long cred4Compare=442;
				unsigned long credValue=header.cred_id;
				if(credValue==cred4Compare)
				{
					rv = fns[header.type](msg);
					num_events++;
				}
